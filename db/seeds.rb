naasei = User.create(user_name: "Naasei", email:"e_owusu@live.com")
anna = User.create(user_name: "Anna", email:"a_hobler@live.com")
jp = User.create(user_name: "Jonathan", email:"jp@live.com")
dustin = User.create(user_name: "Dustin", email:"dtaylor@live.com")
abby = User.create(user_name: "Abigale", email:"ahoke@live.com")
sean = User.create(user_name: "Sean", email:"pearson@live.com")
vlad = User.create(user_name: "Vladimir", email:"vlad@live.com")
madeline = User.create(user_name: "Madeline", email:"moleary@live.com")


jkrowling = Author.create(name: "J K Rolwing")
jrtolkien = Author.create(name: "R Tolkien")
jbond = Author.create(name: "James Bond")

harry_potter = Book.create(name: "Harry Potter")
bible = Book.create(name: "Bible")
moby_dick = Book.create(name: "Moby Dick")
lotr = Book.create(name: "Lord of The Rings")
peter_pan = Book.create(name: "Peter Pan")
relativity = Book.create(name: "Theory of Relativity")
